...
// Đề bài: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/problem/D
...
int query(int idx, int left, int right, int x, int pos) {
    // Kĩ thuật viết SegmentTree walk
    if(right < pos || SegmentTree[idx] < x) return -1;
    if(left == right) return left;
    int mid = (left + right) / 2;
    int res = query(idx * 2 + 1, left, mid, x, pos);
    if(res != -1) return res;
    return query(idx * 2 + 2, mid + 1, right, x, pos);
}
