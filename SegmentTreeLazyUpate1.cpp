...
int A[maxn], SegmentTree[4 * maxn], Lazy[4 * maxn], n, q;
...
void updateRange(int idx, int left, int right, int u, int v, int val) {
    push(idx, left, right);
    ...
    int mid = (left + right) / 2;
    updateRange(idx * 2 + 1, left, mid, u, v, val);
    updateRange(idx * 2 + 2, mid + 1, right, u, v, val); 
    SegmentTree[idx] = SegmentTree[idx * 2 + 1] + SegmentTree[idx * 2 + 2];
}
...
void solve() {
    ...
    while(q--) {
        int type; cin >> type;
        if(type == 1) {
            int left, right, val; cin >> left >> right >> val; // 0 - indexed
            updateRange(0, 0, n - 1, left, right, val);
        } else {
            int left, right; cin >> left >> right; // 0 - indexed
