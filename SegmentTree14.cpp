...
#endif
using namespace std;
int t, n, k, dp[maxn], SegmentTree[4 * maxn];
...
int query(int idx, int left, int right, int u, int v) {
    if(right < u || v < left) return -1e9 - 5;
...
// Quy hoạch động:
// Gọi f[i] là số tiền nhiều nhất tồn tại hộp a[i] - hộp bắt đầu
// Sử dụng các giá trị có trước, ta có công thức quy hoạch động
// dp[i] = max(0, f[i + 1 -> i + k]) + a[i]
// Ta có thể thay thế segment tree bằng deque
...
void solve() {
    ...
    dp[n - 1] = vec[n - 1];
    update(0, 0, n - 1, n - 1, vec[n - 1]);
    for(int i = n - 2; i >= 0; i--) {
        dp[i] = max(0LL, query(0, 0, n - 1, i + 1, min(i + k, n - 1))) + vec[i];
