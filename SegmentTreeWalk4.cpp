...
// Đề bài: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/problem/C
// Dùng Segment Tree Walk
...
void query(int idx, int left, int right, int x) {
    if(x > SegmentTree[0]) {
        cout << -1 << "\n";
        return;
    }
    if(left == right) cout << left << "\n";
        ...
        int mid = (left + right) / 2;
        if(x <= SegmentTree[idx * 2 + 1]) query(idx * 2 + 1, left, mid, x);
        else query(idx * 2 + 2, mid + 1, right, x);
    }
}
